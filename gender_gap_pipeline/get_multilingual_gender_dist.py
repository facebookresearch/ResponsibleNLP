    ...
    if args.hf_datasets is not None:
        ...
        if not args.lang_detect:
            if not len(args.hf_datasets) == len(args.langs):
                # language assumed to be the same for all samples and equal to --langs[0]
                args.langs = [args.langs[0] for _ in args.hf_datasets]
        assert len(args.first_level_key) == len(args.hf_datasets) == len(args.first_level_key)
        if args.second_level_key is not None:
            assert len(args.first_level_key) == len(args.second_level_key)
        for i_dataset, dataset_name in enumerate(args.hf_datasets):
            ...
            reporting(report_df, hb_counter, args.langs[i_dataset] if args.langs is not None else 'Predicted', dataset_name)
