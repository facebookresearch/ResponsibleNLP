...
def eval(args, subject, model, tokenizer, dev_df, test_df):
    ...
    for i in range(test_df.shape[0]):
        ...
        ## This condition is only for models which have less context window and to avoid errors during evaluation
        ## This decreases number of in-context examples to fit the context limit. But it is called for at max 1 or 2 questions for all datasets tested
        while input_ids.shape[-1] > 4096:
            k -= 1
            train_prompt = gen_prompt(dev_df, subject, k)
            prompt = train_prompt + prompt_end
            ...
            torch.nn.functional.softmax(
                torch.tensor(
                    [
                        ...
                        logits[tokenizer("E").input_ids[-1]],
                    ]
                ).float(),
                dim=0,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name",type=str, default="commonsense_qa")
